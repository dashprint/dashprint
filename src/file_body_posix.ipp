...
#ifndef BOOST_BEAST_HTTP_IMPL_FILE_BODY_POSIX_IPP
#define BOOST_BEAST_HTTP_IMPL_FILE_BODY_POSIX_IPP
...
struct basic_file_body<file_posix>
{
    ...
    class writer
    {
        ...
        char buf_[100*1024];    // Small buffer for reading
    ...
    public:
        ...
        writer(message<isRequest,
                basic_file_body<file_posix>, Fields>& m)
            : body_(m.body())
        {
        }
...
public:
    write_some_posix_op(write_some_posix_op&&) = default;
    write_some_posix_op(write_some_posix_op const&) = default;
    ...
    friend
    bool asio_handler_is_continuation(write_some_posix_op* op)
    {
...
operator()()
{
    ...
    size_t const nNumberOfBytesToWrite =
        static_cast<size_t>(
        (std::min<std::uint64_t>)(
            (std::min<std::uint64_t>)(r.body_.last_ - r.pos_, sr_.limit()),
            (std::numeric_limits<size_t>::max)()));

	ssize_t rv = ::sendfile(sock_.native_handle(), sr_.get().body().file_.native_handle(),
							&r.pos_, nNumberOfBytesToWrite);
	if(rv == -1)
	{
		ec.assign(errno, ec.category());
		return 0;
	}
    ...
    error_code& ec)
{
    ...
    size_t const nNumberOfBytesToWrite =
        static_cast<size_t>(
        (std::min<std::size_t>)(
            (std::min<std::size_t>)(r.body_.last_ - r.pos_, sr.limit()),
            (std::numeric_limits<size_t>::max)()));

    ssize_t rv = ::sendfile(sock.native_handle(), r.body_.file_.native_handle(), &r.pos_, nNumberOfBytesToWrite);
    if(rv == -1)
    {
		ec.assign(errno, ec.category());
        return 0;
